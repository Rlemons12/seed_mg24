...
class ApprovedFirmwareCatalog:
    ...
    def list(self) -> list[dict]:
        return [{key: value for key, value in item.items() if key != "artifact_path"} for item in self._packages.values()]
    ...
    def validate(self, package_id: str) -> tuple[dict, Path]:
        ...
        if len(data) != item["artifact_size"] or hashlib.sha256(data).hexdigest().upper() != item["sha256"].upper():
            raise FirmwareValidationError("firmware artifact hash or size mismatch")
        start, end, _ = inspect_intel_hex(path)
        if start != int(item["application_start"], 16) or end != int(item["application_end"], 16):
        ...
        for port in list_ports.comports():
            if port.vid == self.VID and port.pid == self.PID:
                rows.append({"hardware_serial": port.serial_number, "board_type": "Seeed Studio XIAO MG24 Sense",
                             ...
                             "compatible_packages": [item["package_id"] for item in self.catalog.list()]})
        return rows
