...
void SensorChannel::accept_raw(float value, bool valid, uint32_t now) { pending_raw_ = value; pending_valid_ = valid; last_sample_ = now; }
...
void SensorChannel::mark_reported(uint32_t now) { last_reported_ = value_.processed_value; has_reported_ = true; last_report_ = now; }
