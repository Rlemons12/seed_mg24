...
uint32_t rotate_right(uint32_t value, uint8_t bits) {
...
void sha256_compute(const uint8_t* data, size_t length, uint8_t digest[32]) {
  ...
  size_t offset=0;
  while (length-offset>=64) { transform(state,data+offset); offset+=64; }
  uint8_t tail[128]={0};
  ...
  for (size_t i=0;i<8;++i) tail[padded-1-i]=uint8_t(bits>>(i*8));
  transform(state,tail);
  if (padded==128) transform(state,tail+64);
  for (size_t i=0;i<8;++i) {
    digest[i*4]=uint8_t(state[i]>>24); digest[i*4+1]=uint8_t(state[i]>>16);
