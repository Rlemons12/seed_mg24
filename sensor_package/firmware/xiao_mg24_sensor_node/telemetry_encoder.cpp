...
bool encode_heartbeat(uint32_t seq, uint32_t ms, float bv, uint8_t bu, uint32_t dr, uint32_t pe, uint32_t se, const char* id, char* out, size_t size) {
  int written = snprintf(out, size, "{\"t\":\"h\",\"v\":1,\"id\":\"%s\",\"s\":%lu,\"ms\":%lu,\"bv\":%.3f,\"sh\":1,\"bu\":%u,\"dr\":%lu,\"pe\":%lu,\"se\":%lu}", id, (unsigned long)seq, (unsigned long)ms, bv, bu, (unsigned long)dr, (unsigned long)pe, (unsigned long)se);
  return written > 0 && static_cast<size_t>(written) < size;
}
